...
class State:
    ...
    class Enemy(Object):
        ...
        def move(self):
            if not self.x_finish and self.x_range != 0: self.x += self.vel
            elif self.x_finish and self.x_range != 0: self.x -= self.vel
            else: pass

            if not self.y_finish and self.y_range != 0: self.y += self.vel
            elif self.y_finish and self.y_range != 0: self.y -= self.vel
            else: pass


        def change(self):
            if self.x >= self.x_range: self.x_finish = True
            elif self.x <= self.start_x: self.x_finish = False
            else: pass

            if self.y >= self.y_range: self.y_finish = True
            elif self.y <= self.start_y: self.y_finish = False
            else: pass
...
class Room_2(State):
    ...
    def __init__(self):
        self.walls = [Room_2.Wall1, Room_2.Wall2, Room_2.Wall3, Room_2.Wall4, Room_2.Enemy1, Room_2.Enemy2, Room_2.Enemy3, Room_2.tele]
